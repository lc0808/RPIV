...
import com.museu.museu.dto.EditarDivisao;
...
public class DivisaoController {
    ...
    @PutMapping("/{id}")
    @Transactional
    public ResponseEntity<DadosDivisao> editarDivisao(@PathVariable Integer id, @RequestBody EditarDivisao editarDivisao) {
        var divisaoOptional = divisaoRepository.findById(id);

        if (divisaoOptional.isPresent()) {
            var divisao = divisaoOptional.get();

            // Atualize os campos relevantes com base nos dados fornecidos em editarDivisao
            divisao.setNome(editarDivisao.nome());
            divisao.setPredio(editarDivisao.predio());
            divisao.setSala(editarDivisao.sala());

            // Salve a divisao atualizada no repositório
            divisaoRepository.save(divisao);

            cache.put(PEGAR_DIVISAO + id, divisao);

            return ResponseEntity.ok(new DadosDivisao(divisao));
        }
