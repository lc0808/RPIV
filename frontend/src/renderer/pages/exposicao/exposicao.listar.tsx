...
export default function ListarExposicao() {
  const [data, setData] = useState<{ content: Exposicao[] }>({ content: [] });
  ...
  return (
    <div>
      ...
      <div id="funcionarios" className="list-funcionarios">
        {data.content.map((exposicao, index) => (
          // Renderiza o componente FuncionarioComponent para cada funcionário
          <ListComponent key={index} exposicao={exposicao} />
        ))}
      </div>
    </div>
  );
